...
import { Search, Link2, Unlink, ExternalLink, Info, Lock, ArrowRight, RefreshCw } from "lucide-react"
...
export function MyConnectionsContent() {
  ...
  const handleConnectSubmit = () => {
    ...
    setCredentials((prev) => {
      // Remove existing credential for this service
      const filtered = prev.filter((c) => !(c.serviceId === connectDialog && c.userId === user.id))
      return [...filtered, newCredential]
    })
    setConnectDialog(null)
    setSelectedAuthMethod("")
    setTokenInput("")
    toast.success(`${services.find((s) => s.id === connectDialog)?.name}に接続しました`)
  }

  const handleDisconnect = () => {
    if (!disconnectDialog || !user) return
    const serviceName = services.find((s) => s.id === disconnectDialog)?.name
    setCredentials((prev) => prev.filter((c) => !(c.serviceId === disconnectDialog && c.userId === user.id)))
    setDisconnectDialog(null)
    toast.success(`${serviceName}との接続を解除しました`)
  }
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {credential && (
                      <p className="text-xs text-muted-foreground mt-2">
                        {getAuthMethodLabel(credential.authMethod)}で接続 • {credential.connectedAt}
                      </p>
                    )}
                  </div>
                </div>
