...
import { userMcpConnections, userCredentials, type UserMcpConnection } from "@/lib/data"
import { Copy, Check, Key, AlertTriangle, Server, RefreshCw, Link2, ArrowRight } from "lucide-react"
...
export default function McpConnectionPage() {
  ...
  const [reissueDialog, setReissueDialog] = useState(false)
  ...
  // サービス接続状況のサマリー
  const myCredentials = userCredentials.filter((c) => c.userId === user?.id && c.status === "active")
  const connectedServicesCount = myCredentials.length
    ...
    <div className="p-6 space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          {(isGenerated || isRevoked) && (
            <Button variant="outline" onClick={() => setReissueDialog(true)} className="w-full">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle>サービス接続を管理</CardTitle>
              <CardDescription>接続済みのサービスを確認・管理します</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-2xl font-bold">{connectedServicesCount}</p>
              <p className="text-sm text-muted-foreground">サービス接続済み</p>
            </div>
      ...
      <AlertDialog open={reissueDialog} onOpenChange={setReissueDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>トークンを再発行しますか？</AlertDialogTitle>
            <AlertDialogDescription asChild>
              <div className="space-y-3">
                <div className="flex items-start gap-2 p-3 bg-destructive/10 rounded-lg">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="text-muted-foreground mt-1">
                      現在のトークンは即座に無効になります。MCPクライアントの設定を更新する必要があります。
                    </p>
                  </div>
                </div>
              </div>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>キャンセル</AlertDialogCancel>
            <AlertDialogAction onClick={handleGenerateToken} className="bg-destructive text-destructive-foreground">
              再発行する
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
