...
| ID | タスク | 対象ファイル | 備考 |
|----|--------|-------------|------|
| D17-ITR-001 | CLK-GWY仕様更新 | dtl-itr-CLK-GWY.md | TVL→DST、期待する振る舞い追加、v2.0 |
| D17-ITR-002 | CLO-GWY仕様更新 | dtl-itr-CLO-GWY.md | JWT検証フロー詳細化、期待する振る舞い追加、v2.0 |
| D17-ITR-003 | CON-DST仕様更新 | dtl-itr-CON-DST.md | クレジット付与機能追加、enabled_modules/user_prompts/usage_stats明記、v2.0 |
| D17-ITR-004 | CON-PSP仕様更新 | dtl-itr-CON-PSP.md | クレジット前払い制、残高通知、サブスク将来拡張、v2.2 |
| D17-ITR-005 | CON-TVL仕様更新 | dtl-itr-CON-TVL.md | 手動/OAuth登録フロー、トークン検証、Vault暗号化、v2.0 |
| D17-ITR-006 | DST-HDL仕様更新 | dtl-itr-DST-HDL.md | free/paid_credits分離、disabled_tools、consume_credit RPC、v2.0 |
| D17-ITR-007 | EXT-MOD仕様更新 | dtl-itr-EXT-MOD.md | 対応サービス7種、トークンリフレッシュ、エラーハンドリング、v2.0 |
| D17-ITR-008 | HDL-MOD仕様更新 | dtl-itr-HDL-MOD.md | get_module_schema/run/batch詳細、DAG依存、変数参照、v2.0 |
| D17-ITR-009 | HDL-OBS仕様更新 | dtl-itr-HDL-OBS.md | Loki Push API、ツール実行ログ、セキュリティイベント、v2.0 |

---

## dtl-itr-XXX-YYY.md ステータス一覧

### reviewed (20件)

| ファイル | 内容 |
|----------|------|
| dtl-itr-AMW-DST.md | Auth Middleware → Data Store |
| dtl-itr-AMW-GWY.md | Auth Middleware → Gateway |
| dtl-itr-AMW-HDL.md | Auth Middleware → MCP Handler |
| dtl-itr-AUS-CLO.md | Auth Server ↔ Claude |
| dtl-itr-AUS-DST.md | Auth Server → Data Store |
| dtl-itr-AUS-GWY.md | Auth Server → Gateway |
| dtl-itr-AUS-SSM.md | Auth Server → Session Manager |
| dtl-itr-CLK-GWY.md | Claude Code → Gateway |
| dtl-itr-CLO-GWY.md | Claude → Gateway |
| dtl-itr-CON-DST.md | Console → Data Store |
| dtl-itr-CON-EAS.md | Console → External Auth Server |
| dtl-itr-CON-PSP.md | Console → Payment Service Provider |
| dtl-itr-CON-SSM.md | Console → Session Manager |
| dtl-itr-CON-TVL.md | Console → Token Vault |
| dtl-itr-DST-HDL.md | Data Store → MCP Handler |
| dtl-itr-EXT-MOD.md | External API ↔ Modules |
| dtl-itr-HDL-MOD.md | MCP Handler → Modules |
| dtl-itr-HDL-OBS.md | MCP Handler → Observability |
| dtl-itr-IDP-SSM.md | Identity Provider → Session Manager |
| dtl-itr-MOD-TVL.md | Modules → Token Vault |

### draft (5件)

| ファイル | 内容 | 備考 |
|----------|------|------|
| dtl-itr-DST-GWY.md | Data Store → Gateway | APIキー検証 |
| dtl-itr-DST-PSP.md | Data Store → Payment Service Provider | Webhook |
| dtl-itr-DST-SSM.md | Data Store → Session Manager | ユーザー情報 |
| dtl-itr-DST-TVL.md | Data Store → Token Vault | ユーザー紐付け |
| dtl-itr-GWY-OBS.md | Gateway → Observability | HTTPログ |
...
| ID | タスク | 状態 | 備考 |
|----|--------|------|------|
| BL-060 | RFC 8707 Resource Indicators 対応 | 未着手 | `/authorize` と `/token` に `resource` パラメータを追加。Supabase Auth の対応状況を要確認。FIXME: `mcp-client/page.tsx`, `oauth/callback/page.tsx` |
| BL-061 | クレジット初期化を DB トリガーからアプリ層へ移行 | 未着手 | 現在 `handle_new_user` トリガーで credits INSERT を実行しているが、ビジネスロジックのためアプリケーション層（CON→DST）に移行する。FIXME: `00000000000004_user_triggers.sql` |
