...
| ID | タスク | 備考 | 状態 |
|----|--------|------|------|
| D22-006 | `modules/trello/module.go` 作成 | API Key + Token 方式 | |
| D22-007 | Trello Power-Up / API Key 取得 | | |
| D22-008 | `main.go` に RegisterModule 追加 | | |
| D22-009 | Console に Trello API Key 入力 UI 追加 | | |
| D22-010 | 動作確認 | | |
...
- [ ] Todoist モジュールが動作（list_tasks, create_task）
- [ ] Trello モジュールが動作（list_cards, create_card）
