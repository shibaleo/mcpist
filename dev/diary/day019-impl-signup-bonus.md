...
- [x] pre_active ユーザーで billing ページにアクセス → 初回クレジットカードが表示される
- [x] 初回クレジットカードが `animate-pulse-border` で光っている
- [x] 「100クレジットを受け取る」クリック → 100 free_credits 付与
- [x] 付与後、account_status が "active" に変更
- [x] 付与後、初回カードが消えて通常の Stripe カードが表示される
- [x] 既に active のユーザー → 初回カードは表示されない
- [x] 2回クリック（冪等性）→ 「既にクレジットを受け取っています」
