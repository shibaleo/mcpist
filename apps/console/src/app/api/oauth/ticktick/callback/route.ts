...
export async function GET(request: Request) {
  ...
  try {
    ...
    // 認証コードをアクセストークンに交換
    // TickTick uses form-urlencoded with Basic auth
    ...
    const tokenResponse = await fetch(TICKTICK_TOKEN_URL, {
      method: "POST",
      headers: {
    ...
    if (tokenData.error) {
      console.error("TickTick OAuth error:", tokenData.error, tokenData.error_description)
      const errorUrl = new URL(returnTo, request.url)
    ...
    // デフォルトツール設定を保存
    await saveDefaultToolSettings(supabase, "ticktick")

    // 成功時はreturnToにリダイレクト
    const redirectUrl = new URL(returnTo, request.url)
    redirectUrl.searchParams.set("success", "TickTick connected successfully")
