...
export async function GET(request: Request) {
  ...
  try {
    ...
    // デフォルトツール設定を保存
    await saveDefaultToolSettings(supabase, "google_tasks")

    // 成功時はreturnToにリダイレクト
    const redirectUrl = new URL(returnTo, request.url)
    redirectUrl.searchParams.set("success", "Google Tasks connected successfully")
