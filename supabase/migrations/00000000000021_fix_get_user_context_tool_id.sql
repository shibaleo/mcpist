-- =============================================================================
-- Fix get_user_context: tool_name -> tool_id
-- =============================================================================
-- マイグレーション #20 で tool_settings.tool_name を tool_id に変更したが、
-- get_user_context 関数の更新が漏れていた

-- get_user_context: tool_name -> tool_id
...
BEGIN
    ...
    -- 無効なツールを取得（モジュール別）
    -- tool_name -> tool_id に変更
    SELECT COALESCE(
        jsonb_object_agg(m.name, tool_list),
        '{}'::JSONB
    ) INTO v_disabled_tools
    FROM (
        SELECT m.name, array_agg(ts.tool_id) AS tool_list
...
-- public schema wrapper (戻り値型は変わらないのでCREATE OR REPLACEでOK)
