...
|-----------|------|------|
| user_id | ✅ | ユーザーID |
| service_id | ✅ | サービス識別子（notion, google_calendar, microsoft_todo等） |
| auth_type | ✅ | 認証方式（bearer, oauth1, basic, custom_header） |
| credentials | ✅ | auth_typeに応じた認証情報（暗号化保存） |

credentialsの詳細は「トークンデータ構造」セクションを参照。
