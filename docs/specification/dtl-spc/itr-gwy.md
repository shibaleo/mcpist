...
**検証フロー:**
1. GWYがAPI KEYを受信（`Authorization: Bearer mcpist_xxx`）
2. GWYがAPI KEYのSHA256ハッシュを計算
3. TVLにハッシュで検証リクエスト（平文は送信しない）
4. TVLがハッシュに紐づくユーザーIDを返却
5. 検証成功時：ユーザー情報をヘッダーに付与してAMWへ転送
6. GWYはuser_idのみキャッシュ可能（API KEY平文・ハッシュはキャッシュしない）
...
| コンポーネント | 理由 |
|----------------|------|
| MCP Client (API KEY) (CLK) | TVL経由で認証 |
