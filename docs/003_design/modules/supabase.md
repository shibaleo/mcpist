...
- **Total**: 18 tools (GET: 9, POST: 1, SQL composite: 3, URL compute: 1, Composite: 2 + list_tables/list_migrations/apply_migration が runDatabaseQuery にマッピング)
- **ogen operations**: 12
...
| 分類 | ツール | 説明 |
|------|--------|------|
| **ogen 単体** | list_organizations, list_projects, get_project, run_query, get_logs, get_security_advisors, get_performance_advisors, get_api_keys, list_edge_functions, get_edge_function, list_storage_buckets, get_storage_config | ogen client の 1 メソッド呼出 → `toJSON(res)` |
...
### list_migrations の SQL

`supabase_migrations.schema_migrations` テーブルのカラムはプロジェクトによって異なる。
`executed_at` カラムは存在しない場合があるため、`version` と `name` のみを SELECT する。

### generate_typescript_types の削除

Supabase Management API の `/projects/{ref}/types/typescript` は、
レスポンスの Content-Type やボディ形式が不安定で ogen デコードに失敗するケースがあるため削除。
