...
| リスク | 深刻度 | 対策 | 状況 |
|--------|--------|------|------|
| プロンプトインジェクション | 中 | 権限ゲート（多層防御） | 対策済み |
...
MCPクライアント
    ...
    │     ↓
    │   AuthMiddleware → 権限ゲート（通過）
    │     ↓
    │   ModuleSchemaHandler
    │     ├─ GetAllowedTools(userID) → ["notion:search", "notion:get_page"]
    │     ├─ GetSchema("notion") → 全14ツールのスキーマ
    │     └─ filter → 許可された2ツールのスキーマのみ返却（Layer 1）
    │
    └─ tools/call("call_module_tool", {module: "notion", tool: "create_page"})
          ↓
        AuthMiddleware → user_id 抽出
          ↓
        権限ゲート
          ├─ GetAllowedTools(userID) → ["notion:search", "notion:get_page"]
          ├─ "notion:create_page" in allowed? → No → 拒否（Layer 2）
          └─ 権限外呼び出しをログ記録（Layer 3）
```
...
| 権限 | 5分 | Permission Store | ユーザー許可ツール |

- ユーザーあたり約1.5KB（user_id + ツール名リスト）。1000ユーザーで約1.5MB
