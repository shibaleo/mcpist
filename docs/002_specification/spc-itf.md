...
| ヘッダー | 値 | 目的 |
|---------|-----|------|
| `Access-Control-Allow-Origin` | `*` | CORS（MCP クライアントは非ブラウザなので全許可） |
| `X-Content-Type-Options` | `nosniff` | Content-Type sniffing 防止 |
| `X-Frame-Options` | `DENY` | iframe 埋め込み禁止 |
| `Referrer-Policy` | `no-referrer` | Referer ヘッダー送信抑止 |
...
### エラー体系とLLMエラーハンドリングポリシー

MCPist は 3 箇所でエラーを返す: HTTP 層（認証/認可）、JSON-RPC 層（プロトコル）、ToolCallResult 層（ツール実行）。

#### LLM エラーハンドリングポリシー

エラーは **LLM が対応可能かどうか** で 2 カテゴリに分かれる。

**カテゴリ 1: ユーザーに原因を伝えて停止**

認証エラー、認可エラー、クレジット不足、サーバー内部エラーなど、LLM 側では解決できないエラー。HTTP 401/402/403/500 や JSON-RPC エラーの大半がここに該当する。error メッセージにはユーザー向けの案内（管理画面 URL 等）が含まれることがあるため、そのまま伝える。

**カテゴリ 2: LLM が理解して対応を変えるべきエラー**

外部 API エラー（`ToolCallResult.isError: true`）のうち、LLM の次のアクションに影響するもの:

- **404 Not Found** — リソース ID やパスの誤り。パラメータを修正して再試行
- **409 Conflict** — リソース競合（同名が既に存在等）。パラメータを変えて再試行
- **429 Too Many Requests** — レートリミット。時間を置いて再試行
- **401/403 (外部サービス)** — トークン期限切れ。ユーザーに再接続を案内

#### エラーの発生箇所

| 層 | 形式 | 発生元 | 主なケース |
|----|------|--------|-----------|
| HTTP | HTTP ステータス + JSON body | Gateway / Auth Middleware | 認証失敗、認可拒否、クレジット不足 |
| JSON-RPC | JSON-RPC error object | MCP Handler | パース不正、パラメータ不備、認可拒否、内部エラー |
| ToolCallResult | `isError: true` + message | Module (ツール実行) | 外部 API エラー、バリデーションエラー |
...
| -32602 | InvalidParams | パラメータ不備（必須欠落、型不一致、batch サイズ超過） |
| -32601 | MethodNotFound | 未知のメソッド |
| -32603 | InternalError | サーバー内部エラー |
| -32001 | PermissionDenied | モジュール/ツール未有効化 |
| -32002 | InsufficientCredit | クレジット残高不足 |

-32700 〜 -32603 は JSON-RPC 2.0 標準。-32001, -32002 は MCPist 独自（-32000 〜 -32099 はサーバー定義域）。
...
| params | No | ツールパラメータ |
| after | No | 依存タスクID配列 |
| output | No | trueでcompact形式（CSV/MD） |
...
Console は Next.js API Routes (`/api/*`) と Supabase RPC を直接利用する。エンドポイント詳細は実装 (`apps/console/src/app/api/`) が SSoT。
...
| 項目 | 値 |
|------|-----|
| HTTPクライアント | ogen 生成クライアント（公式 OpenAPI spec あり）/ Go標準 `net/http`（spec なし） |
| タイムアウト | 30秒（`modules.Run()` で `context.WithTimeout` を適用） |
| タイムアウト時の挙動 | `ToolCallResult.isError: true` + メッセージ（モジュール名・制限時間を明示） |
...
`broker.GetModuleToken()` が透過的にリフレッシュを実行。`OAuthRefreshConfig` テーブル駆動で 6 プロバイダ (Google, Microsoft, Dropbox, Asana, Todoist, Trello) の 11 モジュールに対応。リフレッシュ条件は `expires_at` が現在時刻 - 5分以内。
