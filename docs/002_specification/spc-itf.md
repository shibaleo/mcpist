...
- `mpt_`: MCPist Token固定プレフィックス
- `<prefix>`: 8文字のランダム文字列（DBでの高速検索用）
- `<random>`: 24文字のランダム文字列

**例:**
```
mpt_abc12345_xxxxxxxxxxxxxxxxxxxxxxxx
```

### ヘッダー形式

```
Authorization: Bearer mpt_abc12345_xxxxxxxxxxxxxxxxxxxxxxxx
```
...
モジュールが外部APIアクセス用のトークン（長期トークン or OAuthトークン）を取得・更新する。

| 項目 | 値 |
|------|-----|
| プロトコル | Supabase PostgREST（RPC） |
| 実装 | PostgreSQL関数 + Vault暗号化 |
| 認証 | service_role key（トークン更新時）またはanon key（トークン取得時） |
...
| `update_module_token` | service_role | トークン更新（リフレッシュ後） |
...
各モジュールがOAuthトークンのリフレッシュを担当する。

**リフレッシュフロー:**

1. モジュールがToken Vault（Supabase RPC）からトークン取得
2. `expires_at` をチェックし、有効期限切れ5分前ならリフレッシュ開始
3. OAuth App認証情報（client_id/client_secret）を`get_oauth_app_credentials` RPCで取得
4. 外部OAuth Provider（Google, Microsoft等）のToken Endpointでリフレッシュ
5. 新しいトークンを`update_module_token` RPCでVaultに保存

**関連RPC:**

| RPC | 権限 | 用途 |
|-----|------|------|
| `get_module_token` | anon/authenticated | ユーザーのトークン取得 |
| `get_oauth_app_credentials` | service_role | OAuth App認証情報取得 |
